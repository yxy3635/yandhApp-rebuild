<template>
  <div class="gomoku-page-wrapper">
    <AppHeader title="破解版五子棋" :showBack="true" :customBack="goBack" />
    <div class="gomoku-main" style="margin-top: 70px;">
        <div id="gomokuRoomList" class="gomoku-room-list" v-show="!inRoom">
            <div class="gomoku-room-list-header">
                <span>房间列表</span>
                <button id="createRoomBtn" class="gomoku-join-btn" @click="createRoom">+ 创建破解房间</button>
            </div>
            <div id="roomListContainer" class="room-list-container">
                ...
                <div v-else v-for="room in rooms" :key="room.id" class="gomoku-room-item">
                    <span class="gomoku-room-name">{{ room.ownerName }}的破解房间</span>
        ...
        <div id="gomokuGameArea" class="gomoku-game-area" v-show="inRoom">
            <div class="gomoku-game-header">
                ...
                <div class="crack-features" style="text-align:center; margin-bottom: 10px;">
                    <span style="background: #ff4d4d; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px;">破解模式</span>
                </div>
            </div>
            ...
            <div class="game-controls" style="display:flex; justify-content:center; gap:10px; margin-top: 16px;">
                <button @click="endGame" style="background:#ff4d4d; color:white; border:none; padding:8px 16px; border-radius:8px; cursor:pointer;">主动结束游戏</button>
                <button @click="clearBoard" style="background:#ffa502; color:white; border:none; padding:8px 16px; border-radius:8px; cursor:pointer;">清空棋盘</button>
            </div>
...
const loadRoomList = async () => {
  ...
  try {
    const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_list_rooms.php`);
...
const createRoom = async () => {
  if (!await customConfirm('确定要创建破解版五子棋房间吗？')) return;
  const roomName = `${username.value}的破解房间`;
  try {
    const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_create_room.php`, {
      method: 'POST',
...
const joinRoom = async (room) => {
  if (room.isMine) {
    ...
    if (!await customConfirm('确定要加入该破解版五子棋房间吗？')) return;
    try {
      const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_join_room.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const fetchAndRenderRoom = async () => {
  try {
    const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_room_status.php?room_code=${currentRoomCode.value}`);
    if (!data.success) return;
    const room = data.room;
    
    // Auto join
    if (String(room.player1_id) !== String(userId.value) && (!room.player2_id || String(room.player2_id) !== String(userId.value))) {
      await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_join_room.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const handleCellClick = async (i, j) => {
  ...
  if (previewX.value === i && previewY.value === j) {
    try {
      const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_move.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const endGame = async () => {
  if (!await customConfirm('确定要主动结束当前游戏吗？')) return;
  try {
    const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_end_game.php`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ room_code: currentRoomCode.value, user_id: userId.value })
    });
    if (data.success) {
      gameOverShown.value = true;
      clearInterval(roomStatusTimer);
      customAlert('你主动结束了游戏！');
      exitRoom();
    } else {
      customAlert(data.message || '结束游戏失败');
    }
  } catch(e) {}
};

const clearBoard = async () => {
  if (!await customConfirm('确定要清空整个棋盘吗？此操作不可撤销。')) return;
  try {
    const data = await commonFetch(`${APP_CONFIG.API_BASE}/gomoku_cracked_clear_board.php`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ room_code: currentRoomCode.value, user_id: userId.value })
    });
    if (data.success) {
      gameMsg.value = '棋盘已清空';
      previewX.value = null; previewY.value = null;
      fetchAndRenderRoom();
    } else {
      customAlert(data.message || '清空棋盘失败');
    }
  } catch(e) {}
};
